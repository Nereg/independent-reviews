...
WITH search AS (
    ...
    FROM unnest(to_tsvector('slovak', UNACCENT($1::text)))
)
...
def search_subject(conn: ConnectionLike, *, search_query: str) -> QueryResults[models.Subject]:
    ...
    def _decode_hook(row: asyncpg.Record) -> models.Subject:
        return models.Subject(id=row[0], name=row[1], facultyId=row[2], aisid=row[3], stage=row[4], semester=row[5], aisCode=row[6], search_vector=row[7])
    return QueryResults[models.Subject](conn, SEARCH_SUBJECT, _decode_hook, search_query)
